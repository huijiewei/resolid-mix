...
export const SliderTrack = forwardRef<HTMLDivElement, Props<'div'>>((props, ref) => {
  ...
  return (
    <>
      <div
        ref={ref}
        className={clsx('relative rounded bg-bg-subtle', vertical ? 'h-full w-1' : 'h-1 w-full', className)}
        {...rest}
      >
        <div
          className={clsx('absolute rounded', vertical ? 'w-full' : 'h-full', disabled ? 'bg-bg-muted' : colorStyle.bg)}
          style={trackStyle}
        />
        {tickMemo && (
          ...
          <div className={vertical ? 'h-full' : 'w-full'}>
            {marks.map((mark) => {
              ...
              return (
                <div
                  key={mark.value}
                  className={clsx('absolute z-[2] flex', vertical ? 'flex-col' : 'flex-row')}
                  style={{
